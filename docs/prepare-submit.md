...
Definitions may carry noun-specific enumerable fields. A method-style `derive` reads them through `this`; the toolkit shallow-snapshots and freezes that full definition at contract creation. The creator infers that exact self type, so direct object-literal authoring such as `prefix: string` plus `this.prefix` compiles while misspelled fields fail TypeScript compilation.

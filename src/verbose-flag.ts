 ...
 */
export function addVerboseFlag(cmd: Command): Command {
  cmd.addOption(
    new Option('-v, --verbose', 'Output full JSON envelope instead of ID only'),
  );
  cmd.addOption(
    new Option('--json', 'Alias for --verbose')
      .hideHelp()
 ...
 */
export function isVerbose(cmd: Command): boolean {
